...
$sqlEntradas = "
    ...
    FROM caixa_entradas
    WHERE instituicao_id = :inst AND data_entrada >= :inicio
    GROUP BY mes
";
$stmt = $db->prepare($sqlEntradas);
$stmt->execute(['inst' => $instituicaoId, 'inicio' => $dataInicioHist]);
...
$sqlSaidas = "
    ...
    WHERE l.instituicao_id = :inst AND p.data_pagamento IS NOT NULL AND p.data_pagamento >= :inicio
    GROUP BY mes
";
...
$sqlProj = "
    ...
    FROM parcelas p
    JOIN lancamentos l ON p.lancamento_id = l.id
    WHERE l.instituicao_id = :inst AND p.data_vencimento >= :inicio AND p.data_vencimento <= :fim
    GROUP BY mes
";
$stmt = $db->prepare($sqlProj);
$stmt->execute(['inst' => $instituicaoId, 'inicio' => $dataInicioProj, 'fim' => $dataFimProj]);
foreach($stmt->fetchAll() as $row) {
